...
public class CraftDataTable {
	 ...
	 * @see #getData(String, String)
	 */
	public void putData(String datasheetName, Map<String, String> keyValues) {
		...
		ExcelDataAccess testDataAccess = new ExcelDataAccess(datatablePath, datatableName);
		testDataAccess.setDatasheetName(datasheetName);
		HSSFWorkbook workbook = testDataAccess.openFileForReading();
		
		int rowNum = testDataAccess.getRowNum(workbook, currentTestcase, 0, 1); // Start
																		// at
																		// row
																		// 1,
																		// skipping
		...
		rowNum = testDataAccess.getRowNum(workbook, Integer.toString(currentIteration), 1, rowNum);
		if (rowNum == -1) {
		...
		if (properties.getProperty("Approach").equalsIgnoreCase("KeywordDriven")) {
			rowNum = testDataAccess.getRowNum(workbook, Integer.toString(currentSubIteration), 2, rowNum);
			if (rowNum == -1) {
...
//				Long.valueOf(properties.getProperty("FileLockTimeOut")));
				...
				// synchronized (this) {
				for(Map.Entry<String, String> map : keyValues.entrySet()) {
					String fieldName = map.getKey();
					String dataValue = map.getValue();
					testDataAccess.setValue(workbook, rowNum, fieldName, dataValue, null);
				}
