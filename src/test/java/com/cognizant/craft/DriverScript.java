...
//		bumblebeeConfig();
		...
		wrapUp();
		//Test Results Push to Azure
		if(Boolean.valueOf(properties.getProperty("UpdateResultsInAzure", "false"))) {					
			AzureIntegration az = new AzureIntegration();
			az.getTestPlanIDAPI();
			az.createTestSuite();
			az.createHostNameSuite(testParameters.getRemoteHostName());
			az.testCaseClone(testParameters.getCurrentTestcase());
			int intTestPoint = az.getTestPoint(testParameters.getCurrentTestcase());
			az.createRun(intTestPoint);
			int intTestResultID = az.createResultsInRun(intTestPoint, testParameters.getRemoteHostName(), getTestStatus());			
			az.addAttachmentToResult(intTestResultID, reportSettings.getReportPath()+Util.getFileSeparator()+"HTML Results"
					+Util.getFileSeparator()+reportSettings.getReportName()+".html", reportSettings.getReportName()+".html", 
					"Automation Test HTML Result for "+testParameters.getCurrentTestcase());
			az.updateTestResult(intTestResultID, getTestStatus());
			az.updateRun(getTestStatus());
			az.updateTestPoint(intTestPoint);
		}
	}
	
	public void bumblebeeConfig() {
		System.setProperty("bumblebee.test.type", "TestNG");
		String strLoggedInUser = System.getProperty("user.name");
		//to be change before handover
		System.setProperty("bumblebee.config.alm_user", "alm_user03");
		System.setProperty("bumblebee.config.alm_password", dataTable.getBumbleebeeConfigValues(strLoggedInUser));		
	}

	public void initializeALMFunctions()
	{
		ALMFunctions = new ALMFunctions(report,testParameters,driver);
	}
	...
	private void initializeWebDriver() {
		...
		case DESKTOP:
			if(testParameters.getPlatform().equals(Platform.MAC)) {
				AppiumDriver<WebElement> mac2Driver = WebDriverFactory.getMac2Driver(testParameters.getAppIdentifier(), 
						testParameters.getRemoteHostIP());
				driver = new CraftDriver(mac2Driver);
				driver.setTestParameters(testParameters);
				break;
			}
			if(testParameters.getPlatform().equals(Platform.WINDOWS)) {
				WindowsDriver<WindowsElement> winDriver = WebDriverFactory.getWindowsDriver(testParameters.getAppIdentifier(),
						testParameters.getRemoteHostIP());
	...
	private void createTestLogHeader() {
		...
		case DESKTOP:
			report.addTestLogSubHeading("Platform", ": " + testParameters.getPlatform(),
					"Execution on", ": " + testParameters.getRemoteHostName()+" {"+testParameters.getRemoteHostIP()+"}");
			break;
	...
	private void initializeTestScript() {
		...
		scriptHelper = new ScriptHelper(dataTable, report, driver, driverUtil, ALMFunctions, apiDriver, extentTest, reusableHandle, testParameters);
		if (!isAPITest()) {
	...
	private void exceptionHandler(Exception ex, String exceptionName) {
		...
		if (ex.getCause() != null) {
			report.updateTestLog(exceptionName, exceptionDescription + " <b>Caused by: </b>" + ex.getCause(),
					exceptionDescription + " <b>Caused by: </b>" + ex.getCause(),
					Status.FAIL);
		} else {
			report.updateTestLog(exceptionName, exceptionDescription, exceptionDescription,
					Status.FAIL);
		}
		...
		report.updateTestLog("Exception stack trace", stackTrace, stackTrace, Status.DEBUG);
	...
	private void wrapUp() {
		...
		closeTestReport();
//		CurrentTest.addAttachment(new File(reportPath + Util.getFileSeparator()+ "HTML Results" 
	...
	private void executeCRAFTLiteTestIterations() {
		while (currentIteration <= testParameters.getEndIteration()) {
			...
			System.out.println("Babu ++ executed");
			// Evaluate each test iteration for any errors
			try {
