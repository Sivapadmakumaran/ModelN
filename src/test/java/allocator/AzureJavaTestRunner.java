 ...
 * @author Cognizant
 */
public class AzureJavaTestRunner {
	...
	public static void main(String[] args) {
		AzureJavaTestRunner azureRunner = new AzureJavaTestRunner();
		azureRunner.driveBatchExecution(args);
	}
	...
	private void driveBatchExecution(String[] args) {
		...
		int testBatchStatus = executeTestBatch(nThreads, args);
	...
	private int executeTestBatch(int nThreads, String[] args) {
		List<SeleniumTestParameters> testInstancesToRun = getRunInfo(frameworkParameters.getRunConfiguration(), args);
	...
	private List<SeleniumTestParameters> getRunInfo(String sheetName, String[] args) {
		
		String strHostName = args[0];
		String strHostIP = args[1];
		List<String> lstTestScriptNames = new ArrayList<String>();
		for (String strTestScript : args[2].split(" ")) {
			lstTestScriptNames.add(strTestScript);
		}
		...
		String[] keys = { "Execute", "TestScenario", "TestCase", "TestInstance", "Description", "IterationMode",
				"StartIteration", "EndIteration", "TestConfigurationID", "AppIdentifier", "RemoteIP" };
		...
		for (int currentTestInstance = 0; currentTestInstance < values.size(); currentTestInstance++) {
			...
			if (lstTestScriptNames.contains(currentTestcase)) {
				...
				testParameters.setRemoteHostName(strHostName);
				testParameters.setRemoteHostIP(strHostIP);
