...
Class DriverScript
	...
	Private Sub InvokeBatchFile()
		Dim strScriptPath
		strScriptPath = m_strRelativePath & "\src\test\java\allocator\RunTest.bat"
		...
		Else
			...
			For i=2 to intRowCount step 1 'First Row Contains Header
				if (Len(objWorkSheet.Cells(i, 5).Value)) then
					...
					objStepFactory.Post
					strStepLevelFolderName = ReadVariablefromProperties(strHomeFolder & "\" & m_strWorkspaceParentFolder & "\" & m_strWorkspaceFolder & "\src\test\resources\Global Settings.properties","StepLevelScreenshotsFolderName")
